# Support

Compass is a personal portable Codex setup shared as a reference. It is
not a general Codex support channel and it is not meant to be installed blindly
on every machine.

Open an issue when the question is about this repository's files, scripts,
manifests, docs, or portability boundaries. Include the command you ran, the
relevant file paths, and the behavior you expected.

Use `SECURITY.md` for secrets, credentials, private machine state, or unsafe
install behavior.

For questions about adapting the setup, describe the change you want and why it
belongs in Compass instead of a local Codex home, target project repo, or plugin
cache.

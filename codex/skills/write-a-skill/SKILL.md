...
- carried but not global capability: create `carried/` with the first item;
...
If the skill is carried but not global, create its `carried/` path in the same
PR, leave it out of installed manifest lists, and document the opt-in route.
Do not retain an empty carried scaffold. A carried path keeps the source out of

...
| Agent measurement tools | Inventory repository prompt surfaces, export content-free structural session traces, and model mechanical compaction accounting | Partial | Medium | Prompt inventory is descriptive only. Raw sessions and generated traces stay in ignored local storage; inspect exports before sharing. Never treat token accounting or a return label as semantic quality or lifecycle authority. |

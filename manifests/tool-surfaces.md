...
| Orchestration ledger | Writes compact controller state through an exclusive lock and atomic file replacement | Partial | Medium | The script is portable; live ledgers stay local under `.local/` or an explicit local path. Gate state records coordination and never grants push, merge, release, deployment, or publication authority. |

...
if (-not (Test-Path -LiteralPath $restartRecoveryScript)) {
    ...
    try {
        ...
        $failedPromptFixture = New-RestartRecoveryFixture -Root (Join-Path $restartRecoveryTemp "failed-prompt") -Lines @(
            ...
            New-RestartRecoveryRecord -Kind "response_item" -Role "user" -Text "Resume the interrupted task. Reopen repository guidance and any named goal, plan, ledger, or checkpoint. Verify current state before continuing."
        )

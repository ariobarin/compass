...
class ReviewBundleTests(unittest.TestCase):
    def test_builds_private_bundle_with_manifest_and_context(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "review-task.md").write_text(
                "Review the user-visible change.\n", encoding="utf-8"
            )
            ...
            self.assertNotIn(
                str(root), (output / "manifest.json").read_text(encoding="utf-8")
            )
    ...
    def test_rejects_secret_like_patch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                module.BundleError, "possible GitHub fine-grained token"
            ):
                module.build_bundle(args(root, parent / "bundle", base))

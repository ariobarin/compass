...
- When removing a previously portable user skill, add an explicit retirement
  entry to `Get-RetiredPortableFileMap` so `install.ps1 -Apply` actually
  uninstalls the live copy from `$HOME/.agents/skills`.

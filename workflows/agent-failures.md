...
first failure: benchmark guidance made invalid rows and documented blockers feel like acceptable stop points, so a timeout row and provider rejection were treated as a run-ending blocker instead of as result artifacts, recovery candidates, or a reason to keep unaffected slices moving
downstream effects: the active runner was stopped after task 29 timed out, task 31 was killed mid-run, the heartbeat was paused, and no naive-arm rows or paired results were produced before the user corrected the priority
